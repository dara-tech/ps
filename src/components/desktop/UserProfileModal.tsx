...
export const UserProfileModal: React.FC<UserProfileModalProps> = ({
  ...
  const [isTouchIdEnrolled, setIsTouchIdEnrolled] = useState(() => {
    try {
      return localStorage.getItem('epr_touch_id_enrolled') === 'true';
    } catch {
      return false;
    }
  });
  ...
  const handleSave = () => {
    ...
    toast.success(
      language === 'kh' ? 'រក្សាទុកជោគជ័យ' : 'Saved',
      language === 'kh' ? 'ព័ត៌មានគណនីត្រូវបានកែសម្រួល' : 'Profile updated successfully'
    );
    onClose();
  };
          ...
          <View style={styles.prefRow}>
            <View style={styles.prefLeft}>
                ...
                {isTouchIdEnrolled ? (
                  <View style={styles.enrolledBadge}>
                    ...
                    <Text style={styles.enrolledText}>
                      {language === 'kh' ? 'បានចុះឈ្មោះ' : 'Enrolled'}
                    </Text>
                  </View>
                ) : (
                  <View style={styles.notEnrolledBadge}>
                    <Text style={styles.notEnrolledText}>
                      {language === 'kh' ? 'មិនទាន់ចុះឈ្មោះ' : 'Not Registered'}
                    </Text>
                  </View>
                )}
              </View>
              <Text style={styles.prefSub}>
                {language === 'kh'
                  ? isTouchIdEnrolled
                    ? 'បានចុះឈ្មោះក្នុងប្រព័ន្ធរួចរាល់ • ផ្ទៀងផ្ទាត់រហ័សសម្រាប់ដោះសោ'
                    : 'មិនទាន់មានក្នុង Database • សូមចុច «ចុះឈ្មោះស្កេន» ដើម្បីបើកដំណើរការ'
                  : isTouchIdEnrolled
                  ? 'Enrolled in database • Fast biometric unlock'
                  : 'Not registered in database yet. Click "Enroll Touch ID" to register.'}
              </Text>
            </View>
            ...
            <View style={styles.biometricActions}>
              ...
              {biometricEnabled && (
                <TouchableOpacity
                  style={[
                    styles.testScanBtn,
                    !isTouchIdEnrolled && styles.enrollScanBtn,
                  ]}
                ...
                >
                  <RemixIcon
                    ...
                    size={12}
                    color={isTouchIdEnrolled ? '#2563EB' : '#16A34A'}
                  />
                  <Text
                    style={[
                      styles.testScanText,
                      !isTouchIdEnrolled && styles.enrollScanText,
                    ]}
                  >
                    {isTouchIdEnrolled
                      ? language === 'kh'
      ...
      <BiometricAuthModal
        ...
        onSuccess={() => {
          ...
          toast.success(
            ...
            language === 'kh'
              ? 'បានចុះឈ្មោះស្កេនម្រាមដៃ Touch ID ក្នុងប្រព័ន្ធរួចរាល់'
              : 'Touch ID enrolled successfully in database.'
          );
        }}
      />
    </CustomModal>
