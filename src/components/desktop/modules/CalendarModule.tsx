...
export const CalendarModule: React.FC = () => {
  ...
  // Silent Background GitHub Auto-Sync on Mount & Settings Change
  React.useEffect(() => {
    if (githubConfig.autoSync && githubConfig.username) {
      syncGithubEvents(true);
    }
  }, [githubConfig.autoSync, githubConfig.username, githubConfig.repo]);
  ...
  const filteredAgendaEvents = selectedDateEvents.filter((e) =>
  ...
  const filteredAgendaTasks = selectedDateTasks.filter((t) =>
    t.title.toLowerCase().includes(agendaSearch.toLowerCase())
  );
      ...
      <View style={styles.topRail}>
        <View style={styles.headerLeft}>
          ...
          <View style={styles.navControls}>
            <TouchableOpacity
              ...
              onPress={viewMode === 'day' ? handlePrevDay : handlePrevMonth}
              activeOpacity={0.7}
            ...
            <TouchableOpacity
              style={styles.arrowBtn}
              onPress={viewMode === 'day' ? handleNextDay : handleNextMonth}
              activeOpacity={0.7}
              hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
            >
        ...
        <View style={styles.headerRight}>
          <View style={styles.viewModeGroup}>
            {(['month', 'day', 'agenda'] as const).map((mode) => (
              <TouchableOpacity
                key={mode}
              ...
              >
                <Text style={[styles.viewModeText, viewMode === mode && styles.viewModeTextActive]}>
                  {mode === 'month' ? t.calMonth : mode === 'day' ? t.calDay : t.calAgenda}
                </Text>
              </TouchableOpacity>
            ))}
          </View>
          ...
          {githubConfig.autoSync && githubConfig.username ? (
            <View style={styles.githubStatusPill}>
              <View style={styles.githubLiveDot} />
              ...
              {calendarDays.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {/* Clean Mini Event Badges with Dot Indicators */}
                    <View style={styles.eventChipsContainer}>
                      {dayEvents.slice(0, 2).map((ev) => {
                        ...
                        return (
                          <View
                            ...
                            style={[
                              styles.eventPill,
                              isUrgent ? styles.pillUrgent : isHigh ? styles.pillHigh : styles.pillNormal,
                            ]}
                          >
                            <View
                              style={[
                            ...
                            <Text
                              style={[
                                ...
                                isUrgent ? styles.pillTextUrgent : isHigh ? styles.pillTextHigh : styles.pillTextNormal,
                              ]}
                              numberOfLines={1}
                            >
                              {ev.title}
                      ...
                      {totalItems > 3 && (
                        <Text style={styles.moreEventsText}>+{totalItems - 3} more</Text>
                      )}
                    </View>
                  </TouchableOpacity>
                );
                        ...
                        <View style={styles.agendaCardTop}>
                          <View style={styles.agendaTypeBadge}>
                            ...
                            <Text style={styles.agendaTypeText}>
                              {ev.type.toUpperCase()} • {ev.time || t.calAllDay}
                            </Text>
                          </View>
          ...
          <View style={styles.agendaViewContent}>
            {calendarEvents.length === 0 ? (
              <View style={styles.agendaEmptyFull}>
                ...
                <Text style={styles.agendaEmptyTitle}>No Scheduled Events</Text>
                <Text style={styles.agendaEmptySub}>
                  Create your first event by clicking "+ New Event" in the top bar.
                </Text>
              </View>
            ) : (
